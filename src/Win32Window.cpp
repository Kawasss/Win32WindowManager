...
Win32Window::Win32Window(const Win32WindowCreateInfo& createInfo)
{
	...
	if (createInfo.windowMode == WINDOW_MODE_BORDERLESS_WINDOWED)
	{
		...
		window = CreateWindowExW((DWORD)createInfo.extendedWindowStyle, className.c_str(), windowName.c_str(), WS_OVERLAPPEDWINDOW, createInfo.x, createInfo.y, createInfo.width, createInfo.height, NULL, NULL, hInstance, NULL);
